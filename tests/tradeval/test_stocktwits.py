"""Tests for tradeval.stocktwits: message parsing and per-symbol scoring."""
...
from tradeval.stocktwits import _parse_time, _to_document, fetch_messages, score_symbols
...
def test_score_symbols_scores_each_ticker_independently():
    ...
    with patch("tradeval.stocktwits.fetch_messages", return_value=[]):
...
def test_score_symbols_marks_unavailable_on_failure():
    ...
    with patch("tradeval.stocktwits.fetch_messages", side_effect=BuzzUnavailable("down")):
