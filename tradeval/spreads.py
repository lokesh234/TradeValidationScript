...
def build_debit_spreads(quotes: Sequence[OptionQuote], count: int = 5) -> List[VerticalSpread]:
    ...
    if len(quotes) < 2:
        return []
    long_leg = quotes[0]
    spreads = [VerticalSpread(long_leg, short) for short in quotes[1 : count + 1]]
    return [s for s in spreads if s.width > 0]
